...
{   
    ...
    while (min <= max)
    {
        ...
        // Return index of the target element if the target is found.
...
{   
    ...
    int center{ min + ((max-min) / 2) };
    if (target < array[center])
        recursiveBinarySearch(array, target, min, center-1);
    else if (target > array[center])
        recursiveBinarySearch(array, target, center+1, max);
    else
        return center;
}
